 ...
 * @param {String} p position:{nw,n,ne}, {ew,e,es}, {sw,s,se}, {wn,w,ws}.
 * @since IE5.0, Firefox1.0, Opera8.0, Safari3.0, Netscape8.0
 * @version 2008/3
 */
var TabControl = function(trigger, p){
    this._tabpages = [];
    this._currTabpage = null;
	this._trigger = trigger||"click";
	this._position = p||"wn"; // position:{nw,n,ne}, {ew,e,es}, {sw,s,se}, {wn,w,ws}.
 ...
 * @return {Number}
 */
TabControl.prototype.activedIndex = function(){
...
TabControl.prototype._Ctl = function(){
	if (!this._d){
		...
		this._d.style.cssText = "margin:0;padding:0px;border:none;";
	    this._table = document.createElement("table");
		this._table.style.cssText = "width:100%;border:0px;border-collapse:collapse;border-spacing:0px;padding:0px;margin:0px;";
		this._table.cellPadding="0px"; this._table.cellSpacing="0px";
		this._tbody = document.createElement("tbody");
		this._t = document.createElement("tr"); // tab.
		this._t.style.cssText = "margin:0px;padding:0px;height:25px;";
		this._space = document.createElement("td");
		this._space.style.cssText="border:0px solid #fff;border-bottom:1px solid #49e;";
		this._space.innerHTML="&nbsp;";
	    this._p = document.createElement("div"); // page.
	    this._p.style.cssText = "border:1px solid #4499ee;border-top:0px solid #fff;height:215px;";

		this._table.appendChild(this._tbody);
		this._tbody.appendChild(this._t);
		this._t.appendChild(this._space);
	    this._d.appendChild(this._table);
		this._d.appendChild(this._p);
	}
	return this._d;
};
...
TabControl.prototype._add = function(t,b,i){
	...
	if ((typeof i === "number" || i instanceof Number) && !isNaN(i) && (i>=0&&i<l)){ // insert.
		...
		if(this._position=="wn"){this._t.insertBefore(t.tab(),this._space);}
		else{this._t.appendChild(t.tab());}
		this._p.appendChild(t.page());
	}
	t.tab().attachEvent("onclick", Function.createDelegate(this, function(){
		this.activize(t);
	}));
 ...
 */
var TabPage = function(t, p, type){
    this._STL = {
		TAB_ON:"width:100px;color:#000;background-color:#fff;text-align:center;padding:0px;margin:0px;display:block;"+
			"font-weight:bold;text-decoration:none;border:1px solid #49e;border-bottom:1px solid #fff;text-decoration:none;",
        TAB_OFF:"width:100px;border:0px solid #fff;background-color:#d6e9fc;color:#49e;border-bottom:1px solid #49e;padding:0px;margin:0px;"+
			"font-weight:normal;text-align:center;text-decoration:underline;cursor:pointer;",
		TAB_DIS:"width:100px;border:1px solid #cccccc;border-bottom:1px solid #cccccc;background-color:#ccc;cursor:default;text-align:center;"+
			"padding:0 .5em 0 .5em;display:block;color:#ccc;font-weight:bold;text-decoration:none;"
    };
...
TabPage.prototype.tab = function(){
	if (!this._t){
	    this._t = document.createElement("td");
		...
		this._t._obj=this;
	    this._t.style.cssText = this._STL.TAB_OFF;
	}
    return this._t;
};
...
TabPage.prototype.on = function(){
	if (this._da) return;
    this._t.style.cssText = this._STL.TAB_ON;
...
TabPage.prototype.setDisable = function(b){
	this._da = b;
	this._t.style.cssText = this._STL.TAB_DIS;
	this._p.style.display = "none";
};
